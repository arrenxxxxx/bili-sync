<script lang="ts">
	...
	async function loadDrama(page: number = 0) {
		loading = true;
		try {
			const response = await api.getFollowedDrama(page + 1, pageSize); // API 使用 1 基索引
			drama = response.data.bangumi;
			totalCount = response.data.total;
		} catch (error) {
			console.error('加载追剧列表失败：', error);
			toast.error('加载追剧列表失败', {
	...
	function handleSubscriptionSuccess() {
		// 重新加载数据以获取最新状态
		loadDrama(currentPage);
	}

	async function handlePageChange(page: number) {
		currentPage = page;
		await loadDrama(page);
	}
